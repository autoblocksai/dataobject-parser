...
describe('DataObjectParser', function () {
  describe('test:unit', function () {
    describe('#set', function () {
      ...
      it('Should take a date object and not change it to a string', function () {
        var d = new DataObjectParser();
        var date = new Date(2014, 0, 1);

        d.set('info', date);
        var newDate = d._data.info;
        expect(_.isDate(newDate)).toBe(true);
        expect(_.isString(newDate)).toBe(false);
      });

      it('Should set objects as values with non-enumerable data', function ($done) {
        var d = new DataObjectParser();
        var dataValue = {};
        Object.defineProperty(dataValue, 'secretKey', {
          value: 123,
          enumerable: false,
        });
        Object.defineProperty(dataValue, 'open', {
          value: function () {
            return this.foo;
          },
          enumerable: false,
        });
        Object.defineProperty(dataValue, 'close', {
          value: function () {
            return this.bar;
          },
          enumerable: false,
        ...
        Object.getOwnPropertyNames(d._data.secretObject).should.eql([
          'secretKey',
          'open',
          'close',
          'publicKey',
        ]);
        d._data.secretObject
          .propertyIsEnumerable('secretKey')
          .should.eql(false);
    ...
    describe('#transpose', function () {
      it('Should take flat data and return structured DataObjectParser', function () {
        var flat = {
        ...
        expect(DataObjectParser.transpose(flat)).toEqual(structured);
      });
      it("Should create a structured data with one key from 'location.name'", function () {
        var flat = {
      ...
      it("Should create a structured data with one key from 'location.address-two'", function () {
        var flat = {
      ...
      it('Should take a date object in flat and it should stay a date object in structured', function () {
        var date = new Date(2014, 0, 1);
      ...
      it("Should create a structured data with one key from 'metadata.foo|bar'", function () {
        var flat = {
      ...
      it("Should create a structured data with one key from 'metadata.foo_bar'", function () {
        var flat = {
